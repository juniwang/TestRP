            ...
            try
            {
                ...
                while (endTime > DateTime.UtcNow)
                {
                    try
                    {
                        ...
                        else
                        {
                            if (response.Content != null)
                            {
                                ...
                                LogUtility.LogError("Error occured in fetching certificates from {0}: {1}", _csmCertificatesFetchUrl, rawException);
                            }
                        }
                    }
                    catch (Exception e)
                    ...
                    if (allTrustedCertThumbprints != null && allTrustedCertThumbprints.Length > 0)
                    {
                        ...
                        LogUtility.LogVerbose("Success. Trusted certificate thumbprints: [{0}]", string.Join(",", allTrustedCertThumbprints));
                        // successfully fetch certificates so return
                        return;
                    }
                    else
                    {
                        LogUtility.LogCritical($"Not able to fetch CSM Certificates from URL: {_csmCertificatesFetchUrl}");
                    }
